...
class ConfigUI(EngineAPI):
    ...
    def _build_device_config_page(self, parent):
        ...
        for idx, key in enumerate(dev_keys):
            self._build_device_frame(dev_frame, key, idx, column=0)
        # 动态计算并强制左右框架宽度比例：左3/5，右2/5
        ...
        # InstrumentManager inst_key -> UI key 映射
        self._inst_key_to_ui = {
            "OSC":          "oscilloscope",
        ...
        tk.Button(btn_box, text="扫描设备", bg="#1E90FF", fg="white",
                  font=("Arial", 9), width=20, command=self._scan_devices).pack(anchor="w", pady=2)
        self._btn_connect = tk.Button(btn_box, text="连接设备", bg="#228B22", fg="white",
                                     font=("Arial", 9), width=20, command=self._connect_devices)
        self._btn_connect.pack(anchor="w", pady=2)
        tk.Button(btn_box, text="断开连接", bg="#DC143C", fg="white",
                  font=("Arial", 9), width=20, command=self._disconnect_devices).pack(anchor="w", pady=2)
    ...
    def _build_device_frame(self, parent, key, row, column=0):
        ...
        check_var = tk.IntVar(value=1)  # 默认勾选
        self._device_check_vars[key] = check_var
        tk.Checkbutton(title_row, variable=check_var,
                       command=lambda k=key: self._on_device_check_changed(k)
                       ).pack(side="left", padx=(0, 4))
        row_f = ttk.Frame(frame)
        row_f.pack(fill="x", pady=1)
        ttk.Label(row_f, text="通信方式", width=10).pack(side="left")
        comm_var = tk.StringVar(value="")
        self._comm_vars[key] = comm_var
        ...
        self._model_vars[key] = model_var
    ...
    def _build_product_info_page(self, parent):
        """产品信息配置页面
        ...
        # 第一行：产品类型勾选 + 带载开机设置（横向排列）
        row1 = ttk.Frame(box_type)
        row1.pack(fill="x", padx=16, pady=(4, 2))

        # 产品类型勾选
        self._prod_type_vars = {}
        for label in ["充电器", "适配器"]:
            var = tk.IntVar(value=0)
            self._prod_type_vars[label] = var
            tk.Checkbutton(row1, text=label, variable=var,
                          onvalue=1, offvalue=0, font=("Arial", 9)).pack(side="left", padx=(0, 12))

        ttk.Separator(row1, orient="vertical").pack(side="left", fill="y", padx=(8, 8))

        # 默认开机电压
        ttk.Label(row1, text="默认开机电压：", font=("Arial", 9)).pack(side="left")
        self._load_startup_voltage_var = tk.StringVar(value="")
        ttk.Entry(row1, textvariable=self._load_startup_voltage_var, width=7).pack(side="left")
        ...
        # 是否带载开机 + 电流
        self._load_startup_var = tk.IntVar(value=0)
        tk.Checkbutton(row1, text="是否带载开机", variable=self._load_startup_var,
                      onvalue=1, offvalue=0, font=("Arial", 9)).pack(side="left")
        self._load_startup_current_var = tk.StringVar(value="")
        ttk.Label(row1, text=" 电流：", font=("Arial", 9)).pack(side="left")
        ttk.Entry(row1, textvariable=self._load_startup_current_var, width=7).pack(side="left")
        ...
        self._input_voltage_lo_var  = tk.StringVar()
        self._input_voltage_hi_var  = tk.StringVar()
        self._output_voltage_var = tk.StringVar()
        self._output_power_var   = tk.StringVar()
        self._power_segment_var = tk.IntVar(value=0)  # 高低压功率分段 勾选
        self._hv_power_var      = tk.StringVar()      # 高压段功率（W）
        self._lv_power_var      = tk.StringVar()      # 低压段功率（W）
        # 产品名称（单框）
        row = ttk.Frame(box_basic)
        row.pack(anchor="w", pady=2)
        ttk.Label(row, text="产品名称：", width=16, font=("Arial", 9)).pack(side="left")
        ttk.Entry(row, textvariable=self._prod_name_var, width=24).pack(side="left")
        # 输入电压范围（双框 ~ 格式）
        row = ttk.Frame(box_basic)
        row.pack(anchor="w", pady=2)
        ttk.Label(row, text="输入电压范围：", width=16, font=("Arial", 9)).pack(side="left")
        ttk.Entry(row, textvariable=self._input_voltage_lo_var, width=12).pack(side="left")
        ttk.Label(row, text=" ~ ", font=("Arial", 9)).pack(side="left")
        ttk.Entry(row, textvariable=self._input_voltage_hi_var, width=12).pack(side="left")
        # 输出电压规格（单框）
        row = ttk.Frame(box_basic)
        row.pack(anchor="w", pady=2)
        ttk.Label(row, text="输出电压规格：", width=16, font=("Arial", 9)).pack(side="left")
        ttk.Entry(row, textvariable=self._output_voltage_var, width=24).pack(side="left")
        # 输出功率规格（单框）
        row = ttk.Frame(box_basic)
        row.pack(anchor="w", pady=2)
        ttk.Label(row, text="输出功率规格：", width=16, font=("Arial", 9)).pack(side="left")
        ttk.Entry(row, textvariable=self._output_power_var, width=24).pack(side="left")
        # 高低压功率分段（勾选后启用HV/LV功率填写框）
        row = ttk.Frame(box_basic)
        row.pack(anchor="w", pady=2)
        tk.Checkbutton(row, variable=self._power_segment_var, onvalue=1, offvalue=0,
                       text="高低压功率分段", font=("Arial", 9),
                       command=self._on_power_segment_toggle).pack(side="left")
        # 高压段功率
        self._hv_power_row = ttk.Frame(box_basic)
        self._hv_power_row.pack(anchor="w", pady=2)
        ttk.Label(self._hv_power_row, text="高压段功率（W）：", width=16, font=("Arial", 9)).pack(side="left")
        self._hv_power_entry = ttk.Entry(self._hv_power_row, textvariable=self._hv_power_var, width=12, state="disabled")
        self._hv_power_entry.pack(side="left")
        # 低压段功率
        self._lv_power_row = ttk.Frame(box_basic)
        self._lv_power_row.pack(anchor="w", pady=2)
        ttk.Label(self._lv_power_row, text="低压段功率（W）：", width=16, font=("Arial", 9)).pack(side="left")
        self._lv_power_entry = ttk.Entry(self._lv_power_row, textvariable=self._lv_power_var, width=12, state="disabled")
        self._lv_power_entry.pack(side="left")
        # 产品规格要求
        box_spec = ttk.LabelFrame(left_panel, text=" 产品规格要求 ", padding=10)
        box_spec.pack(fill="x", padx=8, pady=(4, 8))
        self._spec_vars = {}
        # 6级 / 7级能效：仅勾选框 + 标签，同一行显示
        ...
        for label_text, spec_key in [("6级能效", "6级能效要求（%）"), ("7级能效", "7级能效要求（%）")]:
            ...
            self._spec_vars[spec_key] = {"enable": var_en}
            tk.Checkbutton(row, variable=var_en, onvalue=1, offvalue=0,
        ...
        for label_text in [
            ...
            var_lo = tk.StringVar()
            var_hi = tk.StringVar()
            self._spec_vars[label_text] = {"lo": var_lo, "hi": var_hi}
        ...
        for label_text, key_self, key_latch in [
            ...
            self._prot_vars[label_text] = {"self": var_self, "latch": var_latch}
            tk.Checkbutton(right_box, text="自恢复", variable=var_self,
                          onvalue=1, offvalue=0).pack(side="left", padx=(0, 8))
            tk.Checkbutton(right_box, text="锁死", variable=var_latch,
        ...
        for label_text, default_val in [
            ...
            self._qc_vars[label_text] = {"check": cv, "entry": ev}
            tk.Checkbutton(row, text=label_text, variable=cv,
                          onvalue=1, offvalue=0).pack(side="left")
        ...
        for label_text, default_val in [
            ...
            cv = tk.IntVar(value=0)
            ev = tk.StringVar(value=default_val)
            self._pd_vars[label_text] = {"check": cv, "entry": ev}
            tk.Checkbutton(row, text=label_text, variable=cv,
                          onvalue=1, offvalue=0).pack(side="left")
            ttk.Entry(row, textvariable=ev, width=14).pack(side="right")
        # UFCS
        ...
        for label_text in ["UFCS1档", "UFCS2档", "UFCS3档",
            ...
            self._ufcs_vars[label_text] = {"check": cv, "entry": ev}
            tk.Checkbutton(row, text=label_text, variable=cv,
                          onvalue=1, offvalue=0).pack(side="left")
            ttk.Entry(row, textvariable=ev, width=18).pack(side="right")
    def _build_test_params_page(self, parent):
        """测试参数配置页面
